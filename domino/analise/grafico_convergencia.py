...
def _desenhar_barras(ax, confrontos, data_dir, n, mostrar_rotulos):
    ...
    for i, nome in enumerate(confrontos):
        ...
        if not os.path.exists(arq):
            ...
            ax.text(50, i, "sem dados", ha="center", va="center",
    ...
    ax.set_yticklabels(rotulos_y if mostrar_rotulos else [], fontsize=10)
    ax.set_xlim(0, 100)
    ax.set_xlabel("Taxa de vitória (%)", fontsize=9, color="#555555")
    ...
    ax.set_title(label_n, fontsize=12, fontweight="bold", color="#333333", pad=8)


def gerar_grafico_convergencia(data_dir, output_dir, n_simulacoes=[1_000, 10_000, 100_000]):
    ...
    for grupo_idx, (grupo, sufixo) in enumerate([(grupo1, "1"), (grupo2, "2")]):
        ...
        titulo_grupos = ["Ofensivo vs Defensivo · Ofensivo vs Probabilístico · Ofensivo vs Aleatório",
                         "Defensivo vs Probabilístico · Defensivo vs Aleatório · Probabilístico vs Aleatório"]
        fig.suptitle(f"A taxa de vitória muda com mais simulações?\nConvergência Monte Carlo — {titulo_grupos[grupo_idx]}",
                     fontsize=12, fontweight="bold", color="#222222", y=1.04)
