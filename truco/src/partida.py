...
class Partida:
    ...
    def _jogar_mao(self):
        # Distribui cartas e define a vira
        distribuicao = self.baralho.distribuir(len(self.jogadores), cartas_por_jogador=3)
        vira = self.baralho.vira

        # Cria as mãos passando a vira para avaliação correta
        maos = {j: Mao(distribuicao[i], vira) for i, j in enumerate(self.jogadores)}
        ...
        rodadas_da_mao = []
        for _ in range(3):
            rodada = Rodada(self.jogadores, maos, vira)
            rodadas_da_mao.append(rodada)
            self.rodadas.append(rodada)

            # Verifica se algum jogador quer pedir truco
            for jogador in self.jogadores:
                if jogador.decidir_truco(maos[jogador]):
                    rodada.pedir_truco(jogador)
                    break
